...
import { getMyAccount, updateMyProfile } from "../actions";
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    getMyAccount().then((me) => {
      if (!me) { router.replace("/account/login?next=/account/profile"); return; }
      setF({ name: me.Name || "", phone: me.Phone || "", address: me.Address || "", password: "" });
      setLoaded(true);
    });
  }, [router]);
  ...
  if (!loaded) {
    return <div className="flex justify-center py-20"><div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-primary" /></div>;
  }
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-10">
      ...
      <form onSubmit={save} className="space-y-4">
        ...
        <button disabled={busy} className="w-full bg-primary text-white py-3 rounded-xl font-semibold hover:bg-primary/90 disabled:opacity-50">{busy ? "Saving..." : "Save changes"}</button>
      </form>
    </div>
  );
}
